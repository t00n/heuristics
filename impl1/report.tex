...
In \ref{fig:exercise1_student} (b) we observe that we can reject the null hypothesis for every algorithm except the simplified RZ with exchange and transpose neighbourhoods. In the former cases, the t-statistic is positive so the first-improvement pivoting rule is the best. We cannot conclude anything for the the two latter cases. \\
