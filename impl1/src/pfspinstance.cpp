...
long int PfspInstance::getTime(int job, int machine)
{
    if ((job < 0) || (job >= nbJob) || (machine < 0) || (machine >= nbMac))
      cerr    << "ERROR. file:pfspInstance.cpp, method:getTime. Out of bound. job=" << job
...
bool PfspInstance::readDataFromFile(char * fileName)
{
    ...
    cerr << "name : " << fileNameOK << endl;
    cerr << "file : " << fileName << endl;

    fileIn.open(fileName);

    if ( fileIn.is_open() ) {
        cerr << "File " << fileName << " is now open, start to read..." << std::endl;

        fileIn >> nbJob;
        cerr << "Number of jobs : " << nbJob << std::endl;
        fileIn >> nbMac;
        cerr << "Number of machines : " << nbMac << std::endl;
        cerr << "Allow memory for the matrix..." << std::endl;
        allowMatrixMemory(nbJob, nbMac);
        cerr << "Memory allowed." << std::endl;
        cerr << "Start to read matrix..." << std::endl;
        ...
        cerr << "All is read from file." << std::endl;
        fileIn.close();
    }
    else
    {
        cerr    << "ERROR. file:pfspInstance.cpp, method:readDataFromFile, "
...
long int PfspInstance::computeScore(vector<int> & sol)
{
    ...
    for (m = 1; m < nbMac; ++m)
    {
        previousMachineEndTime[0] +=
                processingTimesMatrix[sol[0]][m];
        previousJobEndTime = previousMachineEndTime[0];
